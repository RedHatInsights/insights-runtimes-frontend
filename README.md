...
First, run the mock host-inventory backend. This repository contains a sub-project that replicates the same mock API that is used in the `insights-inventory-frontend` repository.
...
Second, run the mock runtimes-inventory backend.
...
Finally, run this project using the "mock" setting.
```bash
npm run start:mock
```
